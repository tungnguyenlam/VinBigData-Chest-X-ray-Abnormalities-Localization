...
#let ieee(
  ...
  set text(font: "TeX Gyre Termes", size: 13pt, spacing: .40em)
  // Enums numbering
  ...
  set page(
    ...
    margin: if paper-size == "a4" {
      (x: 45pt, top: 84pt, bottom: 89.51pt)
    } else {
      (
        x: (60pt / 216mm) * 100%,
        top: (60pt / 279mm) * 100%,
        bottom: (68pt / 279mm) * 100%,
      )
    }
  )
  ...
  // Configure lists.
  set enum(indent: 11pt, body-indent: 9pt)
  set list(indent: 11pt, body-indent: 9pt)
  ...
  show heading: it => {
    ...
    if it.level == 1 {
      ...
      set align(left)
      set text(if is-ack { 17pt } else { 17pt })
  ...
  set std.bibliography(title: text(18pt)[References], style: "ieee")
  ...
  set par(justify: true, first-line-indent: (amount: 1em, all: true), spacing: 1em, leading: 0.5em)
